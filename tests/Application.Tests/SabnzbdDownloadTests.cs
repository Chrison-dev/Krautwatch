...
public class AddDownloadByTokenHandlerTests
{
    ...
    public async Task Unknown_token_returns_null_and_enqueues_nothing()
    {
        ...
        var jobId = await new AddDownloadByTokenHandler(episodes, jobs, queue).HandleAsync("nope");
    ...
    public async Task Episode_without_a_stream_returns_null()
    {
        ...
        var jobId = await new AddDownloadByTokenHandler(episodes, jobs, queue).HandleAsync("zdf:2");
